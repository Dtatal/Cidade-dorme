...
export default Evento("messageReactionAdd", async (reaction, user) => {
  if (user.bot) return;
  ...
  const row = query.get();
  if (row == null) return;
  if (!isReactionRoleTable(row)) return error("[events/messageReactionAdd.ts] Invalid table structure.");
  if (reaction.message.partial) await reaction.message.fetch();
  if (reaction.partial) await reaction.fetch();
  const guildUser = await reaction.message.guild.members.fetch(user.id);
  await guildUser.roles.add(row.role_id);
})